...
import app.ember.tv.data.net.DEFAULT_USER_AGENT
...
class PlayerEngine(private val context: Context) {
     ...
     */
    /**
     * @param startPositionMs where to resume a film or episode. Ignored for
     *   live streams, which must start at the live edge rather than at the
     *   beginning of whatever window the server keeps.
     */
    fun play(url: String, referrer: String?, startPositionMs: Long = 0, isLive: Boolean = true) {
        ...
        player?.apply {
            if (!isLive && startPositionMs > 0) setMediaItem(builder.build(), startPositionMs)
            else setMediaItem(builder.build())
            prepare()
            playWhenReady = true
        }
    }
